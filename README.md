...
Stack Orchestrator is a Python3 CLI tool that runs on any OS with Python3 and Docker.
...
```bash
curl -L -o ~/bin/laconic-so https://github.com/cerc-io/stack-orchestrator/releases/download/v1.0.5-alpha/laconic-so
```
...
This will default to ~/cerc or the env var `CERC_REPO_BASE_DIR`

### Build Containers

Builds the set of docker container images required to run a system. It takes around 10 minutes to build all the containers from scratch.
